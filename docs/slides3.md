...
*Slides 8–11 · 23 services across 6 namespaces*

---

### Slide 8 — THEMIS: 23 Services Across 6 Namespaces

**23 platform services governing every AI-assisted analytical interaction from collection requirements through policymaker delivery.**
...
Coalition Liaison                                    CVS / VERITAS
                          ...
                          Outcome Feedback
                                                     IAS / SCUDO
                          UCS / TYCHE                Adversarial Screening
                          Uncertainty
                          Characterization           MAS / EIDOLON
                                                     Media Authenticity
KNOWLEDGE LAYER           AGENT-NATIVE INFRA
──────────────────        ──────────────────         MDS / KRONOS
KCS / ARGUS               SCBS / SENTINEL-CAP        Model Drift Detection
Knowledge Currency        Session Capability
                          Bounding
ERAS / LOGOS                                         INTERACTION LAYER
Reasoning Audit           CBS / BROKER               ─────────────────
                          Credential Broker          PRS / PROMETHEUS
                                                     Prompt Repository
                          RSS / ROLLBACK
                          Reversibility &            SKS / DAEDALUS
                          State Snapshot             Skill Registry

                                                     MGS / TERMINUS
                                                     MCP Gateway
```

*Intelligence cycle completion services (PCS/IRIS, TIS/DIKE) and temporal reasoning (TRS/CHRONOS) are in the Intelligence Layer — see Slide 14.*
...
| **TRUST** | TCS · FGTS · RQS · UCS | Per-analyst calibration, weighted ground truth, retrieval quality, uncertainty characterization (aleatory / epistemic / model) |
...
**Uncertainty Decomposition Indicator** *(Trust — UCS/TYCHE)*  
...
### Slide 14 — From Governance to Intelligence: Five Namespaces
...
|---|---|---|
| **themis-knowledge** | OGS/YGGDRASIL · SCRIBE · MNEMOSYNE · MOS/SAGA | Semantic ontology foundation, document change tracking, institutional knowledge graph, session and matter memory. Prerequisite layer for all other namespaces. |
| **themis-research** | STOA · ORACLE · MIRROR · PYTHIA | Active research orchestration, historical outcome patterns, similar requirement matching, anticipatory surfacing. Builds on knowledge layer; data floors: 50+ reqs (MIRROR), 200+ reqs (ORACLE). |
| **themis-warning** | TRS/CHRONOS · CASSANDRA · ARGUS-LACUNA · LACHESIS · JANUS · SENTINEL | Temporal reasoning, behavioral anomaly detection, collection gap mapping, weak signal fusion, cross-requirement correlation, strategic warning synthesis. Year 3–4. |
| **themis-dissemination** | PCS / IRIS | Policymaker consumer packages — plain-language source basis, falsification indicators, prior outcome record, cryptographic provenance certificate. |
| **themis-requirements** | TIS / DIKE | Upstream requirements integration — gap-to-requirement pipeline from ARGUS-LACUNA, ORACLE effectiveness feedback to requirements officers, full lifecycle tracking. |

**The data floor is real:** themis-research reaches full capability at Year 3–4. themis-warning reaches full capability at Year 4–5. The agencies that build themis-knowledge infrastructure now will have the data to run all five namespaces. The agencies that wait will be starting from zero on both data and infrastructure simultaneously.

---

### Slide 15 — Addressing Unknown Unknowns: The Rumsfeld Layer

**Within `themis-warning`, six services address what we don't know we don't know — from temporal trajectories to strategic surprise.**
...
```
                    themis-knowledge          themis-research
Every session    →  MNEMOSYNE extracts   →   ORACLE / MIRROR
                    tacit knowledge           refine predictions

Every correction →  FGTS enriches corpus  →  calibration improves
Every outcome    →  OFS/NEMESIS closes    →  ORACLE + calibration
                    the loop                  update together

                    themis-warning            themis-requirements
Every anomaly    →  CASSANDRA refines    →   SENTINEL precision
                    baselines                 increases
Every gap signal →  ARGUS-LACUNA        →   TIS/DIKE converts
                    surfaces coverage         to requirements
```
...
| **H2: Intelligence Cycle Completion** | Year 2 | Full intelligence cycle governed. Coalition handling. Policymaker interface. Calibration loop closed. | UCS/TYCHE · CLS/PROTEUS · OFS/NEMESIS · PCS/IRIS · TIS/DIKE · SKS/DAEDALUS + intelligence layer (SCRIBE, STOA, ORACLE, MIRROR, MNEMOSYNE, PYTHIA, YGGDRASIL) |
...
|---|---|---|---|
| **Horizon 1** | Months 1–17 | Team assembly + Phase 1–8 governance build | 23 platform services operational. Full accountability infrastructure. Agent-native capability bounding. Answerable to oversight. |
...
| Analysts share unversioned prompts informally | No version tracking, no standards compliance, no provenance | PRS/PROMETHEUS version hash logged to MOIRAI per session |
| Ungoverned MCP tool calls breach classification boundary | No enforcement layer | MGS/TERMINUS validates every call against SCBS envelope + PCES |
| High-performing analytical skills lost when analyst departs | Institutional knowledge walks out | SKS/DAEDALUS skill registry preserves and rates by outcome data |
