...
## Problem 4: Within-Matter Access Granularity

### Problem Statement

Current PCES/AEGIS enforces binary matter-scoped access. Real matters have internal tiers: courts issue protective orders classifying documents as Confidential, Highly Confidential — Attorneys’ Eyes Only, or Highly Confidential — Source Code. A paralegal may work on a matter but cannot access AEO-designated documents. An outside expert may access a specific evidence subset but nothing else. The existing model has no mechanism for these distinctions.

### Enhancement 4a: Protective Order Tier Model — PCES/AEGIS

PCES/AEGIS moves from binary matter-scoped access to a multi-dimensional access check. Tier is tagged at evidence ingestion. Tier changes (court order modification) produce `AccessRevocationEvent` via the Problem 1 machinery.

```
type ProtectiveOrderTier =
  | 'UNCLASSIFIED'    // No protective order
  | 'CONFIDENTIAL'    // Full matter team
  | 'HC_AEO'         // Attorneys' Eyes Only
  | 'HC_SOURCE_CODE' // Designated attorneys only
  | 'IN_CAMERA'      // Court review only
  | 'SEALED';

access(analyst, evidence) =
  matter_scope_check()
  AND privilege_check()
  AND tier_check(analyst.tier_grants, evidence.tier)
  AND role_check(analyst.matter_role, evidence.tier)
```

**Phase 1-2.** Tier must be tagged at ingestion from day one — retroactive re-classification is operationally expensive. GC-4 role matrix must be finalized before Phase 1-2 begins.

### Role-Tier Access Matrix

|Role                 |CONFIDENTIAL|HC_AEO  |HC_SOURCE_CODE|IN_CAMERA     |
|---------------------|------------|--------|--------------|--------------|
|Supervising Partner  |Yes         |Yes     |By designation|By court order|
|Associate Attorney   |Yes         |Yes     |By designation|By court order|
|Paralegal            |Yes         |No      |No            |No            |
|eDiscovery Specialist|Yes         |No      |No            |No            |
|Outside Expert       |By grant    |By grant|By grant      |No            |
|Client               |By grant    |No      |No            |No            |

Default matrix. Firm-specific role taxonomy requires General Counsel + HR input before Phase 1-2 deployment. See GC-4.

### PGS/NOMOS Enhancement — Tier-Aware Policy

Interaction class policies gain a tier awareness dimension. Export gate thresholds and verification requirements are configurable by protective order tier. Policy rules referencing tier require AI Governance Committee approval.

### Enhancement 4b: Privilege Log Generation — Phase 5-6

PCES/AEGIS already holds everything needed to generate a formal privilege log. Output fields per entry: Document ID, document type and date, author and recipients (from COMMUNICATION participant roles), privilege basis, withheld or redacted-in-part. Accessible from the AI Governance Committee dashboard.

-----

## Problem 5: Temporal and Situational Access

### Problem Statement

Access control is currently static — a point-in-time state. Three temporal dimensions are unaddressed: access rights that are inherently time-bounded, emergency access situations where normal controls would cause operational failure, and the ability to reconstruct what any analyst had access to at any historical moment.

### Enhancement 5a: Time-Limited Access Grants

Access records gain `expires_at` and `grant_type`. On expiration, PCES produces an `AccessRevocationEvent` — connecting time-limited access directly to Problem 1’s blast radius enumeration. Use cases: outside expert engagements, co-counsel joining late, client review windows, seconded attorneys. Notifications at 24h and 1h before expiry. Explicit renewal required.

**Phase 3-4.** Requires AccessRevocationEvent machinery (Problem 1) to be live.

### Enhancement 5b: Break-Glass Emergency Access

|Urgency |Approval Window|No Response            |
|--------|---------------|-----------------------|
|Standard|30 minutes     |Escalate               |
|Urgent  |15 minutes     |Escalate               |
|Critical|5 minutes      |Auto-approve + full log|

Max grant: 24 hours. Mandatory post-hoc review within 5 business days. Reported to AI Governance Committee monthly. ATHENA shows an orange banner: “Break-glass access active — [time remaining].”

**Phase 3-4.**

### Enhancement 5c: Historical Access Reconstruction

PCES/AEGIS gains an append-only AccessStateLog. Every access change produces an `AccessStateChangeEvent`. Query API: `PCES.getAccessStateAt(analyst_id, matter_id, timestamp) → AccessScope`.

Use cases: bar association inquiries, privilege waiver analysis, malpractice defense, conflict retrospectives. Cross-referenceable with MOIRAI provenance records.

**Phase 3-4 alongside MOIRAI.**

### Connection to Problem 1

Time-limited access expiration (5a) and break-glass expiration (5b) both produce `AccessRevocationEvent` — the same event driving Problem 1’s blast radius enumeration and DPS/ERAS marking. These are not parallel systems.

-----

## Updated GC Open Items

**GC-4 — Within-Matter Role-Tier Access Policy · Needed before Phase 1-2 tier deployment**

The role-tier access matrix shows default mappings. The firm’s specific role taxonomy — how associates, paralegals, eDiscovery specialists, and other staff roles are defined, which roles access which tiers, and the process for individual designation grants to HC_AEO and HC_SOURCE_CODE — are firm policy decisions requiring General Counsel input with HR and practice group leadership. Must be finalized before Phase 1-2 tier deployment.

-----

*THEMIS Platform · Addendum B · Access Control Architecture (updated with Problems 4 and 5)*
